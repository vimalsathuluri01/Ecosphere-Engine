...
export function CompareDashboard({ allBrands }: CompareProps) {
    const [selectedNames, setSelectedNames] = useState<string[]>(allBrands.slice(0, 2).map(b => b.Brand_Name));

    const activeBrands = selectedNames.map(name => allBrands.find(b => b.Brand_Name === name)).filter(Boolean) as BrandData[];
    ...
    if (loser.pCarbon && loser.pCarbon < 0.5) loserViolations.push('Carbon Threshold Exceeded');
    if (loser.pWater && loser.pWater < 0.5) loserViolations.push('Water Destabilization Veto');
        ...
        <div className="space-y-8 font-sans">
            ...
            <div className="sticky top-[80px] z-40 bg-[#FAF9F6]/90 backdrop-blur-md py-4 -mx-6 px-6 border-b border-stone-200/50 flex flex-wrap gap-4 items-center">
                ...
                {activeBrands.map((b, idx) => {
                    ...
                    return (
                        <div key={`${idx}-${b.Brand_Name}`} className="flex-1 min-w-[250px] flex items-center bg-white border border-stone-100 rounded-3xl p-3 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative group">
                            <div className="w-12 h-12 rounded-2xl border border-stone-100 flex items-center justify-center font-black text-white text-xl shrink-0" style={{ backgroundColor: BRAND_COLORS[idx] }}>
                                {b.Brand_Name.charAt(0)}
                            </div>
                            <div className="ml-4 flex-1">
                                <select
                                    title={`Select brand ${idx + 1}`}
                                    className="text-xl font-bold text-stone-800 bg-transparent border-none p-0 focus:ring-0 cursor-pointer w-[calc(100%-20px)] uppercase tracking-tighter"
                                    value={b.Brand_Name}
                                    onChange={(e) => handleBrandChange(idx, e.target.value)}
                                >
                                    {allBrands.map(opt => (
                                        <option key={opt.Brand_Name} value={opt.Brand_Name} disabled={selectedNames.includes(opt.Brand_Name) && opt.Brand_Name !== b.Brand_Name}>
                                            {opt.Brand_Name}
                                        </option>
                                    ))}
                                </select>
                                <div className="text-[10px] font-mono text-stone-400 mt-1 uppercase tracking-widest">{b.Category}</div>
                            </div>
                            <div className={cn("px-4 py-2 rounded-2xl font-mono font-black text-xl border", finalScore >= 60 ? "bg-emerald-50 border-emerald-100 text-emerald-600" : finalScore >= 40 ? "bg-amber-50 border-amber-100 text-amber-600" : "bg-rose-50 border-rose-100 text-rose-600")}>
                                {finalScore.toFixed(0)}
                            </div>
                            {selectedNames.length > 2 && (
                                <button title="Remove brand from comparison" onClick={() => removeBrand(idx)} className="absolute -top-2 -right-2 bg-stone-200 hover:bg-rose-500 hover:text-white text-stone-500 rounded-full w-6 h-6 flex items-center justify-center transition-colors opacity-0 group-hover:opacity-100 shadow-sm">
                                    <X className="w-3 h-3" />
                                </button>
                            )}
                        </div>
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <ZenCard className="col-span-12 flex flex-col md:flex-row items-center border-l-8 border-l-emerald-400">
                    <div className="flex-1 mb-6 md:mb-0 md:pr-12">
                        <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-full text-[10px] font-bold uppercase tracking-widest mb-4">
                            <Target className="w-3 h-3" /> Group Leader
                        </div>
                        <h2 className="text-3xl font-black text-stone-800 uppercase tracking-tighter mb-2">
                            {winner.Brand_Name} <span className="text-emerald-500">Leads Pack</span>
                        </h2>
                        <p className="text-stone-500 text-sm">
                            <strong className="text-stone-900">{winner.Brand_Name}</strong> outscores <strong className="text-stone-900">{loser.Brand_Name}</strong> by <strong className="font-mono">{Math.round(scoreDelta)} points</strong>.
                            {loserViolations.length > 0 ? (
                                <span> {loser.Brand_Name} was mathematically vetoed due to <strong className="text-rose-600 uppercase">{loserViolations[0]}</strong>, forcing a systemic penalty.</span>
                            ) : (
                                <span> {loser.Brand_Name} collapsed under raw intensity loads compared to peers.</span>
                            )}
                        </p>
                    </div>
                ...
                <ZenCard className="col-span-12 lg:col-span-6">
                    <HeaderText><Scale className="w-4 h-4" /> The Intensity Spectrum</HeaderText>
                    <p className="text-xs text-stone-500 mb-6">Carbon Intensity (MT per $1M Revenue)</p>
                ...
                <ZenCard className="col-span-12 lg:col-span-6">
                    <HeaderText><AlertTriangle className="w-4 h-4" /> Planetary Boundary Race</HeaderText>
                ...
                <ZenCard className="col-span-12 lg:col-span-8 bg-stone-900 border-none shadow-[0_20px_40px_rgb(0,0,0,0.2)]">
                    <HeaderText><span className="text-emerald-400">The Earth Tax Differential</span></HeaderText>
                    <div className="font-mono text-stone-400 text-xs md:text-sm space-y-4">
                        ...
                        {waterSavedA > 0 && (
                            <div className="p-4 bg-emerald-500/10 rounded-xl mt-4 border border-emerald-500/20 text-emerald-500 leading-relaxed">
                                Equivalent to saving <strong className="text-emerald-300 bg-emerald-900/50 px-1">{Number(poolsSaved).toLocaleString()} Olympic swimming pools</strong> of water annually by adopting the peer group's most efficient practices.
                            </div>
                        )}
                        {isPrimaryBestWater && (
                ...
                <ZenCard className="col-span-12 lg:col-span-4 flex flex-col justify-between">
                    <HeaderText><AlertCircle className="w-4 h-4" /> PR vs Reality Gap</HeaderText>
                        ...
                        {activeBrands.map((b, idx) => (
                            <div key={b.Brand_Name}>
                                ...
                                <div className="flex justify-between mt-1 text-[9px] font-mono text-stone-400">
                                    <span>PR: {b.Transparency_Score_2024}</span>
                                    <span>Actual Survival: {((1 - (b.finalPenalty ?? 0)) * 100).toFixed(0)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <ZenCard className="col-span-12">
                    <HeaderText><TrendingDown className="w-4 h-4" /> Volume vs Value Ratio</HeaderText>
                    <p className="text-xs text-stone-500 mb-6">Revenue per Garment vs Carbon per Garment</p>
                ...
                <ZenCard className="col-span-12 lg:col-span-4">
                    <HeaderText><Target className="w-4 h-4" /> Strategic Policy Shape</HeaderText>
                        ...
                        {activeBrands.map((b, idx) => (
                            <div key={b.Brand_Name}>
                                <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-stone-500 mb-2">
                                    <span style={{ color: BRAND_COLORS[idx] }}>{b.Brand_Name}</span>
                                    <span className="text-emerald-500">{b.Renewable_Energy_Ratio}% Clean</span>
                                </div>
                ...
                <ZenCard className="col-span-12 lg:col-span-4">
                    <HeaderText><Leaf className="w-4 h-4" /> Material vs Waste Generation</HeaderText>
                ...
                <ZenCard className="col-span-12 bg-emerald-50 border-emerald-100">
                    <HeaderText><Users className="w-4 h-4 text-emerald-600" /> The Consumer Shift Protocol</HeaderText>
                        ...
                        <div className="flex-1">
                            {switchCarbonSaved > 0 ? (
                                <p className="text-lg md:text-xl text-stone-700 leading-relaxed font-serif">
                                    If consumers structurally shifted <strong className="text-stone-900 bg-emerald-100 px-1 whitespace-nowrap">just 1%</strong> of {loser.Brand_Name}'s market share exclusively to {winner.Brand_Name}, it would eliminate <strong className="font-mono text-emerald-600 text-3xl px-1">{switchCarbonSaved.toLocaleString(undefined, { maximumFractionDigits: 0 })}</strong> Metric Tons of Carbon from the atmosphere annually.
                                </p>
                            ) : (
                                <p className="text-lg text-stone-700 leading-relaxed font-serif">
                                    A 1% market shift provides entirely negligible structural benefit here, as both bounding entities share severely correlated destructive intensities.
                                </p>
                            )}
                        </div>
                        <div className="shrink-0 text-[10px] font-mono text-emerald-600/60 uppercase tracking-widest bg-emerald-50 p-4 rounded-xl border border-emerald-100 w-full md:w-64">
                            Calculated dynamically based on the specific revenue discrepancy and unit intensity gap between the worst and best performing members of the selected cohort.
                        </div>
                    </div>
                </ZenCard>
